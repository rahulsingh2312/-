...
const SwapYen = ({ isDarkMode = true }) => {
    ...
    const fetchPrice = async () => {
      try {
        const response = await fetch(
          `https://api.jup.ag/price/v2?ids=${YEN_TOKEN_ADDRESS}&vsToken=So11111111111111111111111111111111111111112`
        );
  ...
  return (
    <div
      id="buy"
      className={`flex-col md:my-32 mx-auto my-12 w-fit ${
        isDarkMode ? "text-white" : "text-black"
      }`}
    >
      {/* From Section */}
      <div
        className={`p-4 rounded-3xl min-w-[543px] max-sm:min-w-[310px] ${
      ...
      >
        <div className="w-full flex justify-between items-center px-2 mb-2">
          <h1
            className={`font-medium ${
              isDarkMode ? "text-white/25" : "text-black/50"
            }`}
          >
            Swap from
          </h1>
          <div
            className={`border rounded-full px-2 py-1 flex items-center ${
              isDarkMode ? "border-gray-600" : "border-gray-300"
            }`}
          >
            <span className="mx-1">{fromToken} </span>{" "}
            <img className="w-4 mx-1" src="/solja.webp"></img>
            <CgChevronRight />
          </div>
        </div>
        ...
        >
          <span className="flex">
            Est Fees: {estimatedFees} {fromToken} , cause using{" "}
            <img
              className="mx-2 w-6"
